...
interface ProblemClientProps {
  ...
  initialNote: { content?: string, updatedAt?: string, createdAt?: string } | null;
  problemId: string;
}
...
export default function ProblemClient({ 
  ...
  const [noteUpdatedAt, setNoteUpdatedAt] = useState<string | null>(initialNote?.updatedAt || initialNote?.createdAt || null);
  ...
  useEffect(() => {
    // Determine if we should select a solution initially
    if (initialSolutions.length > 0) {
       loadSolutionById(initialSolutions[0]._id, initialSolutions);
    }
  }, []);
  ...
  const saveNote = async () => {
    ...
    try {
      await fetch('/api/notes', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ problemId: problemId, content: note }),
      });
    } catch (error) {
  ...
  function loadSolutionById(id: string, solutionsList: Solution[] = solutions) {
